...
def cogging_torque_waveform(config: MachineConfig) -> tuple[np.ndarray, np.ndarray]:
    ...
    delta_rad = config.operating.delta_rad + np.deg2rad(rotor_pos_deg)
...
def electromagnetic_torque_waveform(config: MachineConfig) -> tuple[np.ndarray, np.ndarray]:
    """Loaded Eq. (30) waveform with synchronous three-phase excitation."""

    if config.current.I_peak_A is None:
        raise ValueError("electromagnetic torque calculation requires current.I_peak_A")

    period_mech_deg = 360.0 / config.geometry.pole_pairs
    rotor_pos_deg = np.linspace(0.0, period_mech_deg, config.operating.torque_position_count, endpoint=False)
    delta_rad = config.operating.delta_rad + np.deg2rad(rotor_pos_deg)
    theta = np.linspace(0.0, 2.0 * np.pi, config.operating.torque_theta_count, endpoint=False)
    ...
    return rotor_pos_deg, torque_nm
...
def no_load_back_emf_waveform(config: MachineConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    delta_rad = config.operating.delta_rad + np.deg2rad(elec_deg) / config.geometry.pole_pairs
    solved_segments = solve_segments_for_deltas(config, delta_rad)
