...
def save_flux_outputs(
    ...
    loaded = (
        current.I_peak_A is not None
        or current.phase_currents_A is not None
        or current.Jui_A_per_m2 is not None
        or current.Jdi_A_per_m2 is not None
    )
    excitation = "loaded" if loaded else "no_load"

    data_path = output_dir / f"{excitation}_flux_density_{model}_data.npz"
    np.savez_compressed(
        data_path,
    ...
    plot_path = output_dir / f"{excitation}_flux_density_{model}.png"
    fig.savefig(plot_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    return plot_path, data_path


def save_loaded_torque_outputs(
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    data_path = output_dir / "electromagnetic_torque_data.npz"
    np.savez_compressed(
        data_path,
        torque_rotor_pos_mech_deg=torque_deg,
        electromagnetic_torque_Nm=torque_nm,
        I_peak_A=config.current.I_peak_A,
        delta_rad=config.operating.delta_rad,
    )

    fig, ax = plt.subplots(figsize=(7.2, 4.6), constrained_layout=True)
    ax.plot(torque_deg, torque_nm, color="#0072BD", linewidth=2.2, label="Subdomain")
    ax.set_xlabel("Rotor position (Mech.Deg.)")
    ax.set_ylabel("Electromagnetic torque (N.m)")
    ax.set_xlim(float(torque_deg[0]), float(torque_deg[-1]))
    ax.grid(True, alpha=0.35)
    ax.legend(frameon=False)
    torque_path = output_dir / "electromagnetic_torque.png"
    fig.savefig(torque_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    return torque_path, data_path


def save_cogging_outputs(
    torque_deg: np.ndarray,
    ...
    output_dir = Path(config.outputs.performance_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    data_path = output_dir / "cogging_torque_data.npz"
    np.savez_compressed(
        data_path,
        torque_rotor_pos_mech_deg=torque_deg,
...
def save_back_emf_outputs(
    ...
    output_dir = Path(config.outputs.performance_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    data_path = output_dir / "no_load_back_emf_data.npz"
    np.savez_compressed(
        data_path,
