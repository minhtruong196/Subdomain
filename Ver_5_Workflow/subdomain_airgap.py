...
def total_airgap_flux_density(config: MachineConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[float]]:
    ...
    for segment_j in range(1, config.geometry.Nz + 1):
        solution, _, _, layout, meta, residual = solve_boundary_matrix(
            config,
            segment_j=segment_j,
            delta_rad=config.operating.delta_rad,
            # Partition the one physical stator source across PM contributions.
