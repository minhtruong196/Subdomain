...
@dataclass(frozen=True)
class CurrentConfig:
    """Stator excitation inputs. Defaults are the no-load case.

    ``Jui_A_per_m2`` and ``Jdi_A_per_m2`` follow the paper's J_ui and
    J_di notation: one entry per local slot i in the reduced model.
    For loaded torque, set ``I_peak_A`` and let the winding layout generate
    three phase currents and both layer-current-density vectors at every
    rotor position.
    """

    Jui_A_per_m2: tuple[float, ...] | None = None
    Jdi_A_per_m2: tuple[float, ...] | None = None
    # Optional fixed-current snapshot for air-gap checks only.
    phase_currents_A: tuple[float, float, float] | None = None
    # Id = 0 rotating excitation used for loaded torque. For paper rated load:
    # I_peak_A = sqrt(2) * 15.9 A, because Table I gives RMS phase current.
    I_peak_A: float | None = 50
    initial_current_angle_elec_deg: float = 180.0
...
class OperatingConfig:
    ...
    series_turns_per_phase: int = 144       # One layer: 6 turns; two layers: 12 conductors per slot.
    coil_pitch_slots: int = 5               # Short pitch.
    delta_rad: float = 0.0                  # Initial mechanical rotor position.
