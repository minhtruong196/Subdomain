...
def pm_edge_dimensions_mm(config: MachineConfig) -> tuple[float, float]:
    """Return PM side length and upper-edge x-coordinate on the outer profile."""

    edge_zeta_rad = config.geometry.upper_arc_half_angle_rad
    r_u_outer = upper_radius_regular_mm(edge_zeta_rad, config)
    upper_edge_x = r_u_outer * np.sin(edge_zeta_rad)
    upper_edge_y = r_u_outer * np.cos(edge_zeta_rad)
    return float(upper_edge_y - config.geometry.hp_mm), float(upper_edge_x)
...
def edge_radii_mm(edge_zeta_rad: float, config: MachineConfig) -> tuple[float, float]:
    ...
    if config.geometry.edge_radius_mode == "midpoint":
        side_length, upper_x = pm_edge_dimensions_mm(config)
