...
def K_c(config: MachineConfig, Jui: float, Jdi: float) -> float:
...
def slot_current_b04_minus_b03(config: MachineConfig, Jui: float, Jdi: float) -> float:
    Rs = config.stator.R_s_m
    Rsc = config.stator.R_sc_m
    Rsa = config.stator.R_sa_m
    mu0 = config.magnet.mu0
...
def _upper_layer_phase_side(slot_number: int, config: MachineConfig) -> tuple[int, float]:
    """Return phase index and orientation of a double-layer upper coil side."""

    slots_per_belt = config.geometry.slots / (config.geometry.poles * 3)
    if not float(slots_per_belt).is_integer():
        raise ValueError("automatic phase-current winding mapping requires an integral q winding")
    belt = ((slot_number - 1) // int(slots_per_belt)) % 6
    # Spatial sequence for slots 1..Z with the phase-A axis used by the paper.
    return ((0, 1.0), (2, -1.0), (1, 1.0), (0, -1.0), (2, 1.0), (1, -1.0))[belt]


def phase_currents_for_rotor_position(config: MachineConfig, delta_rad: float = 0.0) -> np.ndarray:
    """Return instantaneous (ia, ib, ic) for a rotor mechanical angle Delta."""

    current = config.current
    if current.I_peak_A is None:
        if current.phase_currents_A is None:
            return np.zeros(3)
        return np.asarray(current.phase_currents_A, dtype=float)

    if current.phase_currents_A is not None:
        raise ValueError("set either I_peak_A or fixed phase_currents_A, not both")
    theta_e = config.geometry.pole_pairs * delta_rad + np.deg2rad(current.initial_current_angle_elec_deg)
    return current.I_peak_A * np.cos(theta_e - np.array([0.0, 2.0 * np.pi / 3.0, -2.0 * np.pi / 3.0]))
...
def slot_current_densities(
    ...
    """Return J_ui and J_di for local slots i=1..Z/(2c)."""

    count = config.reduced_slot_count
    current = config.current
    direct_specified = current.Jui_A_per_m2 is not None or current.Jdi_A_per_m2 is not None

    if current.phase_currents_A is not None or current.I_peak_A is not None:
        if direct_specified:
            raise ValueError("set either phase-current excitation or direct Jui/Jdi vectors, not both")

        phase_currents = phase_currents_for_rotor_position(config, delta_rad=delta_rad)
        if phase_currents.shape != (3,):
            raise ValueError("phase_currents_A must contain (ia, ib, ic)")
        if not np.isclose(np.sum(phase_currents), 0.0):
            raise ValueError("phase_currents_A must be balanced: ia + ib + ic = 0")

        # There are Z/3 series coils per phase in a double-layer winding.
        # A half-slot contains one coil side; the two layers together contain
        # twice this number of conductors.
        turns_per_coil = config.operating.series_turns_per_phase / (config.geometry.slots / 3.0)
        upper_area = 0.5 * config.stator.b_s_rad * (config.stator.R_sc_m**2 - config.stator.R_s_m**2)
        lower_area = 0.5 * config.stator.b_s_rad * (config.stator.R_sa_m**2 - config.stator.R_sc_m**2)

        Jui = np.empty(count, dtype=float)
        Jdi = np.empty(count, dtype=float)
        for idx, slot_number in enumerate(range(1, count + 1)):
            phase, sign = _upper_layer_phase_side(slot_number, config)
            Jui[idx] = turns_per_coil * sign * phase_currents[phase] / upper_area

            source_slot = ((slot_number - config.operating.coil_pitch_slots - 1) % config.geometry.slots) + 1
            phase, sign = _upper_layer_phase_side(source_slot, config)
            Jdi[idx] = -turns_per_coil * sign * phase_currents[phase] / lower_area
    else:
        if not direct_specified:
            return np.zeros(count), np.zeros(count)
        if current.Jui_A_per_m2 is None or current.Jdi_A_per_m2 is None:
            raise ValueError("direct current density input requires both Jui_A_per_m2 and Jdi_A_per_m2")
        Jui = np.asarray(current.Jui_A_per_m2, dtype=float)
        Jdi = np.asarray(current.Jdi_A_per_m2, dtype=float)
        if Jui.shape != (count,) or Jdi.shape != (count,):
            raise ValueError(f"Jui/Jdi must each contain {count} local-slot current densities")

    return current_scale * Jui, current_scale * Jdi
...
def build_boundary_matrix(
    ...
    c = config.c_periods
    bs = config.stator.b_s_rad
    Jui, Jdi = slot_current_densities(config, current_scale=current_scale, delta_rad=delta_rad)
    kc = np.array([K_c(config, ju, jd) for ju, jd in zip(Jui, Jdi)])
    b04_rhs = np.array([slot_current_b04_minus_b03(config, ju, jd) for ju, jd in zip(Jui, Jdi)])
    mu0 = config.magnet.mu0
    ...
    for mi, mc in enumerate(mc_values):
        ...
        Y[row] = sum(
            2.0 * c * kc[slot_idx] * f_mi(mc, slot_i, config)
        ...
        Y[row] = sum(
            2.0 * c * kc[slot_idx] * g_mi(mc, slot_i, config)
            for slot_idx, slot_i in enumerate(range(1, layout.slot_count + 1))
        )
        for slot_idx, slot_i in enumerate(range(1, layout.slot_count + 1)):
            for n_idx, n in enumerate(range(1, layout.n_count + 1)):
...
def solve_boundary_matrix(
    ...
    K, Y, layout, meta = build_boundary_matrix(
        config,
        segment_j=segment_j,
        delta_rad=delta_rad,
...
def rhs_for_delta(
    ...
    """Update PM and rotating-current source terms for a rotor angle Delta."""
    ...
    kc = np.array([K_c(config, ju, jd) for ju, jd in zip(Jui, Jdi)])
    b04_rhs = np.array([slot_current_b04_minus_b03(config, ju, jd) for ju, jd in zip(Jui, Jdi)])
    appendix_3_row = 4 * layout.m_count
    for mi, mc in enumerate(mc_values):
        y[appendix_3_row + 2 * mi] = sum(
            2.0 * config.c_periods * kc[slot_idx] * f_mi(mc, slot_i, config)
            for slot_idx, slot_i in enumerate(range(1, layout.slot_count + 1))
        )
        y[appendix_3_row + 2 * mi + 1] = sum(
            2.0 * config.c_periods * kc[slot_idx] * g_mi(mc, slot_i, config)
            for slot_idx, slot_i in enumerate(range(1, layout.slot_count + 1))
        )

    appendix_5_row = 6 * layout.m_count + 2 * layout.an_count
    for slot_idx in range(layout.slot_count):
        y[appendix_5_row + 2 * slot_idx + 1] = b04_rhs[slot_idx]
    return y
...
def solve_segments_for_deltas(config: MachineConfig, delta_rad: np.ndarray) -> list[SegmentSolutionSet]:
    ...
    return [
        solve_segment_for_deltas(
            config,
            segment_j=segment_j,
            delta_rad=delta_rad,
            # Each PM contribution is solved separately; partition the one physical
            # stator source across those linear contributions before superposition.
            current_scale=1.0 / config.geometry.Nz,
        )
        for segment_j in range(1, config.geometry.Nz + 1)
    ]


def solve_segment_for_deltas(
    config: MachineConfig,
    segment_j: int,
    delta_rad: np.ndarray,
    current_scale: float = 1.0,
) -> SegmentSolutionSet:
    """Solve one PM-segment contribution for a vector of rotor positions."""

    K, Y0, layout, meta = build_boundary_matrix(
        config,
        segment_j=segment_j,
        delta_rad=0.0,
        current_scale=current_scale,
    ...
    rhs = np.column_stack(
        [rhs_for_delta(Y0, layout, meta, config, float(delta), current_scale=current_scale) for delta in delta_rad]
    )
    solutions = np.linalg.solve(K, rhs)
    return SegmentSolutionSet(layout=layout, meta=meta, solutions=solutions)
