...
from _geometry import PaperGeometryParams, segment_radii_mm
from _magnetization import MagnetizationParams, magnetization_coefficients
...
class BoundaryMatrixParams:
    ...
    def b_s_rad(self) -> float:
        # Convert straight slot-opening width at Rs to angular slot opening.
...
def xprime_mj_at_ru(
    ...
    A = kprime_mj(R_l, mc, Mr_m, Mt_m, mu0) + mu0 * Mt_m

    pow_lu = np.exp(mc * np.log(R_l / R_u))

    U = (R_l / mc) * pow_lu * A + k_mj(R_u, mc, Mr_m, Mt_m, mu0)

    term_l_prime = -(R_l / R_u) * pow_lu * A

    q_prime_at_ru = mc / R_u * ratio_e_over_p(mc, R_u, R_l)

    return float(term_l_prime + kprime_mj(R_u, mc, Mr_m, Mt_m, mu0) - q_prime_at_ru * U)
