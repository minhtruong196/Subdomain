...
def inferred_pm_side_length_mm(params: PaperGeometryParams) -> float:
    edge_zeta = params.upper_arc_half_angle_rad
    upper_edge_r = upper_radius_regular_mm(edge_zeta, params)
    upper_edge_y = upper_edge_r * np.cos(edge_zeta)
    return float(upper_edge_y - params.hp_mm)


def last_segment_radii_mm(params: PaperGeometryParams) -> tuple[float, float]:
    """Return Ru, Rl for the special side segment.

    Local convention: O is rotor center, d-axis is +y,
    x = r sin(zeta), y = r cos(zeta).
    """

    edge_zeta = params.upper_arc_half_angle_rad
    upper_edge_r = upper_radius_regular_mm(edge_zeta, params)
    upper_edge_x = upper_edge_r * np.sin(edge_zeta)
    upper_edge_y = upper_edge_r * np.cos(edge_zeta)
    inferred_side_length_mm = upper_edge_y - params.hp_mm

    # The final segment is special in the paper: Ru(Nz) is measured from O
    # to the center of the PM side. Fig. 3 shows this side parallel to the
    # d-axis, so its lower endpoint is at the same x on y = hp.
    Ru = np.hypot(upper_edge_x, params.hp_mm + 0.5 * inferred_side_length_mm)

    # Rl(Nz) is represented independently by the midpoint of the lower
    # boundary portion of the side segment, not by Rl(edge_zeta).
    lower_edge_x = lower_radius_regular_mm(edge_zeta, params) * np.sin(edge_zeta)
    if upper_edge_x < lower_edge_x:
        raise ValueError("last lower segment cannot be inferred: PM side is inside the radial edge")
    Rl = np.hypot(0.5 * (lower_edge_x + upper_edge_x), params.hp_mm)
...
def segment_radii_mm(params: PaperGeometryParams) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for idx, j in enumerate(orders[:-1]):  
        ...
        Ru[idx] = upper_radius_regular_mm(zeta, params)
        Rl[idx] = lower_radius_regular_mm(zeta, params)

    Ru[-1], Rl[-1] = last_segment_radii_mm(params)
...
def main() -> None:
    ...
    print(f"inferred PM side length [mm] = {inferred_pm_side_length_mm(params):.6f}")
    print()
    print("j, zeta_deg/side, Ru_mm, Rl_mm")
    for j, ru, rl in zip(orders, Ru, Rl):
        angle = f"{np.rad2deg(zeta_j_rad(int(j), params)):9.5f}" if j < params.Nz else "     side"
        print(f"{j:2d}, {angle}, {ru:8.4f}, {rl:8.4f}")
    print()
